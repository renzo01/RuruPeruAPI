...
namespace VentasServices
{
    ...
    public class Service1 : Ventas
    {
        ...
        public string ActualizarStock(ItemProducto reg)
        {
            ...
            {
                return mensaje = "El producto pedido no tiene una cantidad valida";
            }

            // 20 > 20
            int stockProductoSeleccionado = ListarProductos().Where(x => x.idProducto == reg.idProducto).FirstOrDefault().stockProducto;

            if (reg.cantidadProducto > stockProductoSeleccionado)
            {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("usp_actulizar_stock_producto", cn);
        ...
        public List<Producto> ListarProductos()
        {
            ...
            SqlCommand cmd = new SqlCommand("usp_listado_productos", cn);
            cmd.CommandType = CommandType.StoredProcedure;
            cn.Open();
        ...
        public string RealizarVenta(string idCliente, List<ItemProducto> items)
        {
            ...
            try
            {
                ...
                foreach (var it in items)
                {
                    ...
                    cmd = new SqlCommand("usp_registrar_Detalle_Factura", cn);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@idEst", idEstadoFactura);
        ...
        string generarIdFactura()
        {
            string cod = "";
            SqlCommand cmd;
            cmd = new SqlCommand("usp_maximo_num_facturas", cn);
        ...
        int generarIdEstadoFactura()
        {
            int cod = 0;
            SqlCommand cmd;
            cmd = new SqlCommand("usp_num_facturas", cn);
            cmd.CommandType = CommandType.Text;
            cn.Open();
